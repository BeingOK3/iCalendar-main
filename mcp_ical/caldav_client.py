...
class CalDAVManager:
    ...
    def create_event(self, new_event: CreateEventRequest) -> Event:
        ...
        if not calendar:
            ...
            # 尝试找到一个可写的日历
            writable_calendar = None
            for cal in calendars:
                try:
                    # 检查日历是否可写（通过检查日历属性）
                    logger.debug(f"Checking calendar: {cal.name}")
                    writable_calendar = cal
                    break
                except Exception as e:
                    logger.debug(f"Calendar {cal.name} may not be writable: {e}")
                    continue
            calendar = writable_calendar or calendars[0]  # Use first writable or just first available
        ...
        try:
            ...
            if "Forbidden" in error_msg or "403" in error_msg:
                raise ValueError(
                    f"无法在日历 '{calendar.name}' 中创建事件：权限被拒绝。"
                    f"该日历可能是只读的或者是订阅日历。"
                    f"请尝试在其他日历中创建事件。"
                ) from e
            elif "Unauthorized" in error_msg or "401" in error_msg:
                raise ValueError(
                    "日历访问未授权。请检查您的 Apple ID 和应用专用密码是否正确。"
                ) from e
            else:
                raise ValueError(f"创建事件失败: {error_msg}") from e
